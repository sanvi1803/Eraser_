...
function SideNavTopSection({ user, setActiveTeamInfo }: any) {
  ...
  return (
    <div>
      <Popover>
        <PopoverTrigger>
          <div className="flex items-center gap-3 hover:bg-slate-200  duration-200 p-3 rounded-lg cursor-pointer">
            <Image src="/logo-1.png" alt="logo" width={40} height={40} />
            <h2 className="flex gap-2 items-center font-bold text-[17px]">
              {activeTeam?.teamName}'s Team <ChevronDown />
            </h2>
          </div>
        </PopoverTrigger>
        <PopoverContent className="ml-7 p-4">
          ...
          <div>
            {teamList?.map((team, index) => (
              <h2
                className={`p-2 hover:bg-blue-500 hover:text-white rounded-lg mb-1 cursor-pointer duration-100 ${activeTeam?._id === team._id && "bg-blue-500 text-white"}`}
                key={index}
